 ...
 */
private static RadixSortA(String[] arr, int stringLen) {
...
/** Counting radix dort an array of String
 * Assume all are all ASCII
 * Assume all have same length
 */
public static void countingRadixSort(String[] arr, ing stringLen) {
    final int BUCKETS = 256;
    ...
    for (int pos = stringLen - 1; pos >= 0; pos--) {
        ...
        for (int i = 0; i < N; i++)
            count[in[i].charAt(pos) + 1]++;  // count[k+1] 为桶k中元素的个数
        
        for (int b = 1; b <= BUCKETS; b++)
            count[b] += count[b - 1];       // 值严格小于k的元素的个数
